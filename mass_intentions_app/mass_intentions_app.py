...
def parse_intentions_pdf(file_bytes):
    ...
    with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
        for page in pdf.pages:
            ...
            try:
                d = dt.strptime(day_name, '%A %B %d %Y').date()
            except Exception:
                d = date.today()
...
with st.expander("📂 Import from existing PDF", expanded=False):
    st.caption("Upload a previously generated OLOG Mass Intentions PDF to pre-fill the form. You can then add or edit names and regenerate.")
    ...
    if uploaded_pdf is not None:
        col_load, col_merge = st.columns([1, 1])
        with col_load:
            if st.button("⬆ Load PDF (replace all days)", type="primary"):
                with st.spinner("Parsing PDF…"):
                    try:
                        parsed = parse_intentions_pdf(uploaded_pdf.read())
                        if parsed:
                            st.session_state.days = parsed
                            st.success(f"✅ Loaded {len(parsed)} day(s) from PDF.")
                            st.rerun()
                        else:
                            st.warning("No days found in that PDF. Make sure it was generated by this app.")
                    except Exception as e:
                        st.error(f"Could not parse PDF: {e}")
        with col_merge:
            if st.button("➕ Merge into existing days", type="secondary"):
                with st.spinner("Parsing PDF…"):
                    try:
                        parsed = parse_intentions_pdf(uploaded_pdf.read())
                        if parsed:
                            ...
                            st.warning("No days found in that PDF.")
                    except Exception as e:
                        st.error(f"Could not parse PDF: {e}")
