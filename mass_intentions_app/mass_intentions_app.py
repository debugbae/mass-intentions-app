...
[data-testid="stSidebar"] { background-color: #EDE8DC !important; }
...
label, .stCheckbox label, .stSelectbox label { color: #1a1a1a !important; }
...
uploaded_pdf = st.file_uploader(
    "📂 Import from existing PDF",
    type=["pdf"],
    key=f"pdf_upload_{st.session_state.pdf_key}",
)
if uploaded_pdf is not None:
    st.caption(f"Selected: **{uploaded_pdf.name}**")
    if st.button("➕ Import this file", type="primary"):
        with st.spinner("Parsing PDF…"):
            try:
                parsed = parse_intentions_pdf(uploaded_pdf.read())
                if parsed:
                    st.session_state.days.extend(parsed)
                    st.session_state.pdf_key += 1  # resets the uploader widget
                    st.rerun()
                else:
